<template>
  <div>
    <p class="text-sm pl-2 pb-1 mb-2">
      Total Number of Active clusters: <b v-if="data?.list">{{ data.list.length }}</b
      ><i v-else>loading</i>.
    </p>
    <p class="text-sm pl-2 pb-2 mb-3">
      Number of clusters that contain one or more documents with "Reference Not Received":
...
const generatedAt = computed(() =>
  data.value?.timestampIso ? DateTime.fromISO(data.value.timestampIso) : undefined
)
...
const stats = computed((): Record<StatKeys, number> => {
  ...
  const totalClustersWithDocumentsReferenceNotReceived = data.value.list
    .map((item) => item.documents.some((doc) => !doc.isReceived))
    .reduce((acc, clusterHasNotReceived) => acc + (clusterHasNotReceived ? 1 : 0), 0)
